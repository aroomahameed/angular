...
export class SelectComponent implements ControlValueAccessor, OnInit  {
  ...
  writeValue(obj: any): void {
    this.formControlDirective.valueAccessor.writeValue(obj && obj.id ? obj.id : obj);
  }
  ...
  getName(item: any) {
    if (typeof this.name === typeof []) {
        return this.name.reduce((value, key) => value == null ? '' : value[key], item);
    }

    return item && item[this.name] != null ? item[this.name] : '';
