...
export class MaterialFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.material) {
      this.model = this.store.select(selectByClientID, this.relationId(this.material.client))
      this.article = this.store.select(selectByModelsID, this.relationId(this.material.model))
      this.sizes = this.store.select(selectByArticleID, this.relationId(this.material.article))
      this.productionlines = this.store.select(selectByArticlesizeID, this.relationId(this.material.size))
    }
    if (this.productionline) {
      this.model = this.store.select(selectByClientID, this.relationId(this.productionline.client))
      this.article = this.store.select(selectByModelsID, this.relationId(this.productionline.model))
      this.sizes = this.store.select(selectByArticleID, this.relationId(this.productionline.article))
      this.productionlines = this.store.select(selectByArticlesizeID, this.relationId(this.productionline.size))
    }
  ...
  private buildForm() {
    ...
    this.fg = this.fb.group({
      ...
      client: [this.productionline ? this.relationId(this.productionline.client) : this.material ? this.relationId(this.material.client) : null, Validators.required],
      model: [this.productionline ? this.relationId(this.productionline.model) : this.material ? this.relationId(this.material.model) : null, Validators.required],
      article: [this.productionline ? this.relationId(this.productionline.article) : this.material ? this.relationId(this.material.article) : null, Validators.required],
      size: [this.productionline ? this.relationId(this.productionline.size) : this.material ? this.relationId(this.material.size) : null, Validators.required],
      production_line: [this.productionline ? this.productionline.id : this.material ? this.relationId(this.material.production_line) : null, Validators.required],
      material: [this.material ? this.relationId(this.material.material) : null, Validators.required],
      serial_no: [this.material ? this.material.serial_no : ''],
      base_uom: [{ value: this.material ? this.relationId(this.material.base_uom) : null, disabled: true }],
  ...
  async onChangeMaterial(event) {
    ...
    let sub = this.allmaterial.subscribe(data => {
      ...
      this.fg.patchValue({ width: mat.mat_width || 0 })
      this.fg.patchValue({ base_uom: this.relationId(mat.base_uom) })
      this.fg.patchValue({ length: mat.mat_length || 0 })
      this.fg.patchValue({ height: mat.mat_height || 0 })
