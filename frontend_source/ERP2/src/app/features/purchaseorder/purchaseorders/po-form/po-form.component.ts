...
import { Purchaseorder, addPurchaseorder, addPurchaseorderSuccess, selectCurrentPurchaseorderSuccess, selectCurrentPurchaseorder } from '../../../store/purchaseorder/purchaseorder';
...
export class PoFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.purchaseorder) {
      ...
      this.currentOrder.pipe(take(1)).subscribe(data => {
        if (data) {
  ...
  constructor(private modalService: BsModalService, private fb: FormBuilder, private fb1: FormBuilder, private store: Store<any>, private actions$: Actions) {
  }
  ...
  onSubmit(event) {
    ...
    this.actions$.pipe(
      ofType(addPurchaseorderSuccess),
