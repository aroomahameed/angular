...
export class DetailFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.posub) {
      this.poheads = this.store.select(selectByPOID, this.relationId(this.posub.production_order))
      this.posubs = this.store.select(selectByPOSubID, this.posub.id)
      this.sizes = this.store.select(selectByArticleID, this.relationId(this.posub.article))
    }
    if (this.podetail) {
      this.poheads = this.store.select(selectByPOID, this.relationId(this.podetail.production_order))
      this.posubs = this.store.select(selectByPOSubID, this.relationId(this.podetail.production_order_sub))
      this.sizes = this.store.select(selectBySizeID, this.relationId(this.podetail.article_size))
    }
  }
  private buildForm() {
    this.fg = this.fb.group({
      id: [this.podetail ? this.podetail.id : null],
      production_order: [this.posub ? this.relationId(this.posub.production_order) : this.podetail ? this.relationId(this.podetail.production_order) : null, Validators.required],
      production_order_sub: [this.posub ? this.posub.id : this.podetail ? this.relationId(this.podetail.production_order_sub) : null, Validators.required],
      article_size: [this.podetail ? this.relationId(this.podetail.article_size) : null, Validators.required],
  ...
  private buildForm1(productionline, material) {
    this.fh = this.fb1.group({
      ...
      production_order: [this.reservepo ? this.relationId(this.reservepo.production_order) : this.relationId(this.podetail.production_order)],
      production_order_sub: [this.reservepo ? this.relationId(this.reservepo.production_order_sub) : this.relationId(this.podetail.production_order_sub)],
      production_order_Detail: [this.reservepo ? this.relationId(this.reservepo.production_order_Detail) : this.podetail.id],
      po_number: [this.reservepo ? this.reservepo.po_number : this.podetail.production_order.po_number],
      client: [this.reservepo ? this.relationId(this.reservepo.client) : this.relationId(this.podetail.production_order.client)],
      customer: [this.reservepo ? this.relationId(this.reservepo.customer) : this.relationId(this.podetail.production_order.customer)],
      identifier: [this.reservepo ? this.relationId(this.reservepo.identifier) : this.relationId(this.podetail.production_order.identifier)],
      factory: [this.reservepo ? this.relationId(this.reservepo.factory) : this.relationId(this.podetail.production_order.factory)],
      ...
      article: [this.reservepo ? this.relationId(this.reservepo.article) : this.relationId(this.podetail.production_order_sub.article)],
      model: [this.reservepo ? this.relationId(this.reservepo.model) : this.relationId(this.podetail.production_order_sub.model)],
      article_size: [this.reservepo ? this.relationId(this.reservepo.article_size) : this.relationId(this.podetail.article_size)],
  ...
  reserveMat(event) {
    ...
    let productionline = this.store.select(selectByArticlesizeID, this.relationId(this.podetail.article_size))
          ...
          material.subscribe(data2 => {
            for (let j = 0; j < data2.length; j++) {
              this.store.select(selectByPO, {
                client: this.relationId(this.podetail.production_order.client),
                customer: this.relationId(this.podetail.production_order.customer),
                model: this.relationId(this.podetail.production_order_sub.model),
                article: this.relationId(this.podetail.production_order_sub.article),
                size: this.relationId(this.podetail.article_size),
                prdline: data[i].id,
                material: data2[j].id
              }).subscribe(dataa => {
                if (dataa.length > 0) {
