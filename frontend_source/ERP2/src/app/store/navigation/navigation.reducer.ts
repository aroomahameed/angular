...
function toggleItems(navItems: NavigationItem[], toggledItem: NavigationItem): NavigationItem[] {
  ...
  const isToggledItemLevel = navItems.some(_ => sameNavItem(_, toggledItem));
  return navItems.map(navItem => {
    ...
    if (isToggledItemLevel && item.items && !sameNavItem(navItem, toggledItem)) {
      item.open = false;
    }
    if (sameNavItem(navItem, toggledItem)) {
...
function sameNavItem(a: NavigationItem, b: NavigationItem): boolean {
  return a.title === b.title && a.routerLink === b.routerLink && a.tags === b.tags;
}
